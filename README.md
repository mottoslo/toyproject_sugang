...
`신민철`
- [x]  ~~classlist에서 자료가 호출되지 않음~~
<br>→ user_id를 선언하여 id 기준으로 자료를 호출하여 해결
<br>→ list자료를 호출하여 for문을 사용하여 해결
- [x]  ~~수강신청 버튼이 작동하지 않음~~
<br>→ temp_html에 ${class_code}로 해결
<br>→ ’${class_code}’의 ‘’로 문자를 string로 해결
- [x]  ~~관심강의 삭제버튼이 작동하지 않음~~
<br>→ temp_html에 ${class_code}로 해결
<br>→ ’${class_code}’의 ‘’로 문자를 string로 해결
- [ ]  체크박스를 체크해서 신청 및 삭제
<br>→ 추후 수정 예정
...
**<후기>** <br>
첫 팀프로젝트였고 github, sourcetree를 처음 사용하여 우리들의 자료를 하나로 모와서 하나의 프로젝트를 완성했다는 것이 너무 뿌듯하고 흥미로웠다. <br>
